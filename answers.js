...
var answerObject = {
	...
	checkRepeat: function(){
		...
		for (var i=0; i < this.allGuesses.length; i++){
			if (main.toAnswers.userLetter == this.allGuesses[i]){
				repeatCounter++;
			}
		}
	...
	checkMatch: function (){
		...
		for (var i=0; i < game.toAnswers.wordLetters.length; i++){
			if (main.toAnswers.userLetter == game.toAnswers.wordLetters[i]){
				matchCounter++;
			}
		}
	...
	checkMatchRepeat: function(){
		...
		if (this.isRepeat == true){
			this.allGuesses.pop(main.toAnswers.userLetter);
		...
		if (this.isRepeat == false && this.isMatch == false){
			this.incorrectGuesses.push(main.toAnswers.userLetter);
		...
		if (this.isRepeat == false && this.isMatch == true){
			this.correctGuesses.push(main.toAnswers.userLetter);
			this.guessesRemaining--;
...
exports.toCharacters = answerObject;
exports.toMain = answerObject;
